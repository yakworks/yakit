...
import { requestAnimationFrame, getTranslate } from '../../shared/utils.js';
...
export default function pickerColumn(colEl, updateItems) {
  ...
  col.setValue = function setColValue(newValue, transition, valueCallbacks) {
    ...
    const newTranslate = -newActiveIndex * itemHeight + maxTranslate; // Update wrapper
  ...
  col.updateItems = function updateColItems(activeIndex, translate, transition, valueCallbacks) {
    ...
    const selectedItem = col.items.eq(activeIndex).addClass('picker-item-selected').transform(''); // Set 3D rotate effect
  ...
  function handleTouchEnd() {
    ...
    const activeIndex = Math.round(Math.abs((newTranslate - maxTranslate) / itemHeight)); // Normalize translate
